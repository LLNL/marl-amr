...
class TransitionGraph(typing.NamedTuple):
  nodes_t: Optional[np.ndarray]
  edges_t: Optional[np.ndarray]
  adj_t: Optional[np.ndarray]
  n_t: Optional[int]
  a_t: Optional[np.ndarray]
  r_t: Optional[float] # change to ndarray if multi-objective
  nodes_tp1: Optional[np.ndarray]
  edges_tp1: Optional[np.ndarray]
  adj_tp1: Optional[np.ndarray]
  n_tp1: Optional[int]
  done: Optional[bool]
...
class PrioritizedTransitionReplay(Generic[ReplayStructure]):
  ...
  def sample(
      self,
      size: int,
  ) -> Tuple[List[ReplayStructure], np.ndarray, np.ndarray]:
